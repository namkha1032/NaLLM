MERGE (:`Group` {import_name: 'Ballas'  })
MERGE (:`Group` {import_name: 'Triads'  })
MERGE (:`Group` {import_name: 'Merryweather'  })
...
MERGE (:`Person` {import_name: 'Brad', `status`: 'imprisoned'  })
...
MERGE (:`Person` {import_name: 'Dave Norton', `status`: 'alive', `occupation`: 'FIB agent'  })
...
MERGE (:`Person` {import_name: 'Floyd', `status`: 'deceased', `relation`: 'cousin', `occupation`: 'associate'  })
...
MERGE (:`Person` {import_name: 'Steve Haines', `status`: 'deceased', `occupation`: 'corrupt FIB agent'  })
MERGE (:`Person` {import_name: 'Devin Weston', `status`: 'deceased', `occupation`: 'billionaire'  })
MERGE (:`Person` {import_name: 'Solomon Richards', `occupation`: 'Vinewood producer'  })
MERGE (:`Person` {import_name: 'Patricia', `status`: 'alive', `occupation`: 'kidnapped', `relationship`: 'wife'  })
...
MATCH (source { import_name: 'bank robbery crew' }), (target { import_name: 'Merryweather Security' }) MERGE (source)-[:`attempted robbery` {`year`: '2004'}]->(target);
...
MATCH (source { import_name: 'Franklin' }), (target { import_name: 'Devin Weston' }) MERGE (source)-[:`follows orders` {}]->(target);
MATCH (source { import_name: 'Franklin' }), (target { import_name: 'Michael De Santa' }) MERGE (source)-[:`kills` {}]->(target);
MATCH (source { import_name: 'Franklin' }), (target { import_name: 'Trevor' }) MERGE (source)-[:`reaches out to` {}]->(target);
MATCH (source { import_name: 'Trevor' }), (target { import_name: 'Michael De Santa' }) MERGE (source)-[:`expresses disinterest in meeting with` {`reason`: 'considering him a traitor'}]->(target);
MATCH (source { import_name: 'Franklin' }), (target { import_name: 'Michael De Santa' }) MERGE (source)-[:`chases` {}]->(target);
MATCH (source { import_name: 'Michael De Santa' }), (target { import_name: 'bridge' }) MERGE (source)-[:`falls to his death at` {}]->(target);
MATCH (source { import_name: 'Michael De Santa\'s family' }), (target { import_name: '{}' }) MERGE (source)-[:`expresses knowledge of Franklin\'s involvement in Michael De Santa\'s death` {}]->(target);
MATCH (source { import_name: 'Michael De Santa\'s widow' }), (target { import_name: 'Franklin' }) MERGE (source)-[:`messages` {`message`: 'stay away from her family'}]->(target);
...
MATCH (source { import_name: 'Trevor' }), (target { import_name: 'Michael De Santa' }) MERGE (source)-[:`concocts a plan with` {}]->(target);
MATCH (source { import_name: 'Michael De Santa' }), (target { import_name: 'Ballas' }) MERGE (source)-[:`takes out the leader of` {}]->(target);
...
MATCH (source { import_name: 'Dave Norton' }), (target { import_name: '{grateful: True}' }) MERGE (source)-[:`wipes clean their records` {}]->(target);
