...
from utils.unstructured_data_utils import myDisambiguation
...
async def root(payload: ImportPayload):
    ...
    try:
        ...
        if not payload.neo4j_schema:
            ...
            final_result = extractor.run(data=payload.input)
        else:
            extractor = DataExtractorWithSchema(llm=llm)
            final_result = extractor.run(schema=payload.neo4j_schema, data=payload.input)

        # print("Begin disambiguation...")
        # disambiguation = DataDisambiguation(llm=llm)
        # disambiguation_result = disambiguation.run(final_result["neo_result"])
        mydis = myDisambiguation(final_result["my_result"])

        return {"dis": mydis, "nodis": final_result["neo_result"], "nodismy":final_result["my_result"], "mydis": mydis}
